...
const IncomingOrderEditModal = (props) => {
  ...
  const [destination, setDestination] = useState(
    singleIncomingOrder.destination ?? ""
  );
  const [product, setProduct] = useState(singleIncomingOrder.product ?? "");
  const [dateCreated, setDateCreated] = useState(
    singleIncomingOrder.dateCreated ?? ""
  );
  const [productOrderID, setProductOrderID] = useState(
    singleIncomingOrder.productOrderID ?? ""
  );
  const [truckNo, setTruckNo] = useState(singleIncomingOrder.truckNo ?? "");
  const [wayBillNo, setWayBillNo] = useState(
    singleIncomingOrder.wayBillNo ?? ""
  );
  const [transporter, setTransporter] = useState(
    singleIncomingOrder.transporter ?? ""
  );
  ...
  const updateQantity = (e, data) => {
    ...
    if (findID === -1) {
      ...
      cloneSelected[findID] = {
        ...cloneSelected[findID],
        incomingQuantity: e.target.value,
      };
      setSelected(cloneSelected);
