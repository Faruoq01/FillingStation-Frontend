...
const UpdatePayments = (props) => {
  ...
  useEffect(() => {
    ...
    setBankName(
      props.data.bank === "bank"
        ? props.data.data.bankName
        : props.data.data.posName
    );
    setTellerID(
      props.data.bank === "bank"
        ? props.data.data.tellerNumber
        : props.data.data.terminalID
    );
    setBankAmount(props.data.data.amountPaid);
  }, [
    props.data.bank,
    props.data.data.amountPaid,
    props.data.data.anountPaid,
    props.data.data.bankName,
    props.data.data.posName,
    props.data.data.tellerNumber,
    props.data.data.terminalID,
  ...
  const submit = () => {
    ...
    if (tellerID === "")
      return swal("Warning!", "Teller or ID cannot be empty", "info");
    if (bankAmount === "")
      return swal("Warning!", "Amount paid cannot be empty", "info");
    setLoading(true);
    ...
    APIs.post(
      "/comprehensive/update-payment",
      type === "bank" ? payload : payload2
    )
      .then(() => {
        ...
        <div style={{ height: "auto", margin: "20px" }} className="inner">
          <div className="head">
            <div className="head-text">
              Edit {type === "bank" ? "Bank Payment" : "POS Payment"}
            </div>
            <img
              onClick={handleClose}
          ...
          <div className="middleDiv" style={inner}>
            <div style={{ marginTop: "15px" }} className="inputs">
              ...
              <div style={{ marginTop: "15px" }} className="inputs">
                <div className="head-text2">
                  {type === "bank" ? "Teller ID" : "Terminal ID"}
                </div>
                <OutlinedInput
                  sx={{
                    width: "100%",
