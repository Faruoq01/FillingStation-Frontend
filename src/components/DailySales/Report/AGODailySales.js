...
const AGODailySales = (props) => {
  ...
  const getMasterRows = () => {
    ...
    if (dailySales?.AGO?.sales) {
      for (let row of dailySales?.AGO?.sales) {
        ...
        if (findID === -1) {
          const filterUniqueRows = dailySales.AGO.sales.filter(
            (data) => data.pumpID === row.pumpID
          );
          const filterLPORows = dailySales.AGO.lpo.filter(
            (data) => data.pumpID === row.pumpID
          );
          const filterRTRows = dailySales.AGO.rt.filter(
            (data) => data.pumpID === row.pumpID
          );
          ...
          const totalRTPrice = filterRTRows.reduce((accum, current) => {
            return (
              Number(accum) + Number(current.rtLitre) * Number(current.AGOPrice)
            );
          }, 0);
    ...
    const uniq = Array.from(new Set(newRows.map((s) => s.id))).map((id) => {
      return {
  ...
  const deleteAGORecord = (data) => {
    swal({
      ...
      if (willDelete) {
        DailySalesService.deleteSales({ id: data._id })
          .then((data) => {
            props.refresh();
          })
          .then(() => {
              ...
              dailySales?.AGO?.sales?.map((data, index) => {
                return (
                  <div key={index} style={tableHeads2}>
                    ...
                    <div style={cols}>{data.AGOSellingPrice}</div>
                    <div style={cols}>
                      {data.AGOSellingPrice *
                        (Number(data.closingMeter) - Number(data.openingMeter))}
                    </div>
