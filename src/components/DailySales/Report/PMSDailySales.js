  ...
  const getMasterRows = () => {
    ...
    if (dailySales?.PMS?.sales) {
      for (let row of dailySales?.PMS?.sales) {
        ...
        if (findID === -1) {
          const filterUniqueRows = dailySales.PMS.sales.filter(
            (data) => data.pumpID === row.pumpID
          );
          const filterLPORows = dailySales.PMS.lpo.filter(
            (data) => data.pumpID === row.pumpID
          );
          const filterRTRows = dailySales.PMS.rt.filter(
            (data) => data.pumpID === row.pumpID
          );
          ...
          const totalRTPrice = filterRTRows.reduce((accum, current) => {
            return (
              Number(accum) + Number(current.rtLitre) * Number(current.PMSPrice)
            );
          }, 0);
      ...
      <div style={sales}>
        ...
        <div style={mainSales}>
          <div style={inner}>
            ...
            {dailySales?.PMS?.sales?.length === 0 ? (
              <div style={dats}> No Data </div>
            ) : (
              dailySales?.PMS?.sales?.map((data, index) => {
                return (
