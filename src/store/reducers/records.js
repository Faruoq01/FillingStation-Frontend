    ...
    case SELECTED_PUMPS: {
      const updateSelectedPumps = () => {
        ...
        const findID = currentList.findIndex(
          (data) => data._id === payload.selected._id
        );
      ...
      const updateSelectedTanks = () => {
        ...
        const findID = currentList.findIndex(
          (data) => data._id === payload.tank._id
        );
      ...
      return {
        ...state,
        PMS:
          payload.selected.productType === "PMS"
            ? payload.collection
            : state.PMS,
        AGO:
          payload.selected.productType === "AGO"
            ? payload.collection
            : state.AGO,
        DPK:
          payload.selected.productType === "DPK"
            ? payload.collection
            : state.DPK,
    ...
    case DESELECT_PUMP_LIST: {
      ...
      const removeTank = () => {
        ...
        const availableID = currentList.filter(
          (data) => data.hostTank === payload.tank._id
        );
